...
import { api, fetchDefaultModels, saveDefaultModel } from '@/shared/api';
import { DEFAULT_MODELS_CHANGED_EVENT } from '@/shared/constants';
import type { DefaultModelMap, LLMProvider, ProviderModelsDefinition } from '@/shared/types';
...
export function useDefaultModelSettings() {
  // Tracks modelCatalog for the default-model settings control.
  const [modelCatalog, setModelCatalog] = useState<Partial<Record<LLMProvider, ProviderModelsDefinition>>>({});
  // Tracks defaultModels for the default-model settings control.
  const [defaultModels, setDefaultModels] = useState<DefaultModelMap>({});
  // Tracks loading for the default-model settings control.
  const [loading, setLoading] = useState(true);
  // Tracks savingProvider for the default-model settings control.
  const [savingProvider, setSavingProvider] = useState<LLMProvider | null>(null);
  // Tracks error for the default-model settings control.
