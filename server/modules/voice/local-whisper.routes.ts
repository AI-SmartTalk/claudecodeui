...
import { asyncHandler, findApplicationRoot, getModuleDirectory } from '@/shared/utils.js';

const APP_ROOT = findApplicationRoot(getModuleDirectory(import.meta.url));
...
// Config keys read by the voice module's defaults (see voice.module.ts).
...
function run(args: string[], timeoutMs = 20_000): Promise<ComposeResult> {
...
async function waitForHealthy(timeoutMs = 60_000): Promise<boolean> {
  const deadline = Date.now() + timeoutMs;
  while (Date.now() < deadline) {
    try {
      const response = await fetch(`http://127.0.0.1:${WHISPER_PORT}/health`, {
...
router.get('/status', asyncHandler(async (_req, res) => {
  ...
  const dockerAvailable = version.ok
    || !/not found|command not found|not recognized/i.test(version.stderr);
  res.json({
    dockerAvailable,
...
router.post('/enable', asyncHandler(async (_req, res) => {
...
router.post('/disable', asyncHandler(async (_req, res) => {
  ...
  res.json({
    ...
    running: false,
    configured: false,
    details: down.ok ? null : down.stderr.trim(),
  });
}));

export const localWhisperRoutes = router;
