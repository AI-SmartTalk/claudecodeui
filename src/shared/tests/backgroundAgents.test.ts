...
import { formatElapsed, getRunningBackgroundAgents } from '@/shared/utils';
...
const subagentMessage = (
  ...
  subagent: { id: `agent-${description}`, status: isComplete ? 'completed' : 'running' },
  subagentActivity: Array.from({ length: toolCount }, (_unused, index) => ({
    kind: 'tool', toolId: `child-${index}`, toolName: 'Read', toolInput: {},
  })),
});
