...
/** Used by project-workspace to manage the selected project’s Compose services. */
export function DockerPanel({ selectedProject, isVisible }: DockerPanelProps) {
  // Stores the latest Compose service status.
  const [services, setServices] = useState<DockerService[]>([]);
  // Indicates a service refresh is pending.
  const [loading, setLoading] = useState(false);
  // Tracks whether the selected project has Compose configuration.
  const [hasCompose, setHasCompose] = useState(true);
  // Tracks whether the server can access Docker.
  const [dockerAvailable, setDockerAvailable] = useState(true);
  // Displays the latest Docker request failure.
  const [error, setError] = useState<string | null>(null);
  // Identifies the pending service action to disable its control.
  const [busy, setBusy] = useState<string | null>(null); // `${action}:${service||'*'}`
  // Keeps the requested log output visible until dismissed.
  ...
  const loadServices = useCallback(async () => {
    ...
    setError(null);
    try {
      const res = await api.docker.services(projectPath);
  ...
  const run = useCallback(
    async (action: DockerAction, service?: string) => {
      ...
      setBusy(key);
      try {
        const res = await api.docker.run({ action, projectPath, service });
        const data = await res.json();
        if (action === 'logs') {
