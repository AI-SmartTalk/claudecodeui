...
export type ClaudeQueryHandle = AsyncIterable<ClaudeStreamMessage> & {
  ...
  /** Stops one background task; the CLI answers with a `stopped` notification. */
  stopTask?(taskId: string): Promise<void>;
  /** Ends the CLI process. Scripted streams in tests may not have one. */
  close?(): void;
};
...
export type ClaudeLiveProcessOptions<TTurn> = {
  ...
  /**
   * Recognises a tool call that leaves work behind without reporting a task
   * (a scheduled wake-up, a monitor). Only consulted for turns that reported
   * no task events: when a turn did, those events are the whole truth.
   */
  startsDeferredWork?(message: ClaudeStreamMessage): boolean;
};
...
// Tool calls that leave work to happen after the turn. Used when the runtime
// supplies no rule of its own; a turn that reports task events is judged on
// those instead.
...
export class ClaudeLiveProcess<TTurn> {
  ...
  /** Tasks the CLI reported starting and has not settled yet. */
  private readonly backgroundTaskIds = new Set<string>();
  /** Epoch ms the process was spawned; launches older than this belong to an earlier one. */
  ...
  private sawTaskEventThisTurn = false;
  ...
  /**
   * Stops one background task. The CLI reports it `stopped` and pushes no
   * turn to relay it, which is what releases the process if it was the last.
   * @returns False when the process has ended or cannot stop tasks
   */
  async stopTask(taskId: string): Promise<boolean> {
    if (this.finished || typeof this.query.stopTask !== 'function') {
      return false;
    }
  ...
  private track(message: ClaudeStreamMessage): void {
    ...
    const detectsDeferredWork = this.options.startsDeferredWork ?? startsDeferredWork;
    if (message.type === 'assistant' && detectsDeferredWork(message)) {
    ...
    switch (message.subtype) {
      case 'task_started':
        // Housekeeping tasks the CLI starts on its own carry no tool call and
        // nothing waits on them. A foreground task settles before its turn
        // ends, so tracking every other one only ever holds for real work.
        if (taskId && typeof message.tool_use_id === 'string') {
          this.backgroundTaskIds.add(taskId);
          this.sawTaskEventThisTurn = true;
        }
        break;
      case 'task_updated':
  ...
  private react(message: ClaudeStreamMessage): void {
    ...
    if (message.type === 'result') {
      ...
      // until the next turn — the one that work triggers — has run. A turn that
      // reported its tasks is judged on those alone: an Agent call without
      // `run_in_background` looks deferred to the static rule, but ran in the
      // foreground and settled before this result.
      this.holdForDeferredWork = this.sawTaskEventThisTurn ? false : this.deferredWorkThisTurn;
      this.deferredWorkThisTurn = false;
      this.sawTaskEventThisTurn = false;
    ...
    if (message.type === 'system' && message.subtype === 'task_notification' && message.status === 'stopped') {
      // A stopped task gets no turn to relay it, so nothing else would ever
      // end the hold if it was the last thing outstanding.
      this.holdForDeferredWork = false;
      this.reconsiderHold();
      return;
    }
